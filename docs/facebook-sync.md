...
Pulls the latest posts from the official LGU Santa Maria Facebook Page, categorizes
...
1. **Get an Editor role on the page.** Ask the LGU Santa Maria page admin (via Meta
   Business Suite → Settings → People) to add the BetterSantaMaria account/app as an
...
| Name              | Type         | Value                                               |
| ----------------- | ------------ | --------------------------------------------------- |
| `FTP_SERVER`      | Secret       | cPanel FTP host (e.g. `ftp.bettersantamaria.org`)       |
